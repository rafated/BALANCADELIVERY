...
#Retorna uma string com a data e hora de execução da função
def get_string_time():
    current_time = datetime.datetime.now()
    time_string = current_time.strftime('%Y%m%d_%Hh%Mm%Ss')
    return time_string

#Filtra e armazena os dados recebidos pela conexão serial
def process_data(data, recibo, count, config):
    ...
    if "\\x1bd\\x1dVB\\r" in data1:    
        ...
        # Escrever no arquivo
        #URGENTE Colocar em uma função também (rotina repetida)
